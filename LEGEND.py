...
# Configuration
TOKEN = "8111340428:AAF8OdA3cuV_o-ujPEHMvd7Sh9miML6vgDA"  # Replace with your Telegram bot token
ADMIN_IDS = {7462351545}  # Replace with your actual admin user ID(s)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data is None or user_data.get("expiration_date") < datetime.now():
        await update.message.reply_text("ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴛᴏ ᴜsᴇ ᴛʜɪs ʙᴏᴛ")
        return
    
    keyboard = [[KeyboardButton("ᴀᴛᴛᴀᴄᴋ 🚀")]]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text("ᴘʀᴇss ᴛʜᴇ ᴀᴛᴛᴀᴄᴋ ʙᴜᴛᴛᴏɴ ᴛᴏ sᴛᴀʀᴛ ᴄᴏɴғɪɢᴜʀɪɴɢ ᴛʜᴇ ᴀᴛᴛᴀᴄᴋ.", reply_markup=reply_markup)
...
async def approve_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴛᴏ ᴀᴘᴘʀᴏᴠᴇ ᴜsᴇʀs.")
        return

    if plan_value not in [100, 200]:
        await update.message.reply_text("ɪɴᴠᴀʟɪᴅ ᴘʟᴀɴ. ᴘʟᴇᴀsᴇ ᴜsᴇ 100 ᴏʀ 200.")
        return
...
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        keyboard = [
            [KeyboardButton("sᴛᴀʀᴛ ✅"), KeyboardButton("sᴛᴏᴘ ❌")],
            [KeyboardButton("ʀᴇsᴇᴛ 💤")]
        ]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
